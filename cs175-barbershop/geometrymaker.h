...
// A generic vertex structure containing position, normal, and texture
// information Used by make* functions to pass vertex information to the caller
struct GenericVertex {
    ...
    GenericVertex(float x, float y, float z, float nx, float ny, float nz,
                  float tu, float tv, float tx, float ty, float tz, float bx,
                  float by, float bz)
...
#define DEFV(x, y, z, nx, ny, nz, tu, tv)                                      \
    {                                                                          \
        *vtxIter = GenericVertex(x h, y h, z h, nx, ny, nz, tu, tv, tan[0],    \
                                 tan[1], tan[2], bin[0], bin[1], bin[2]);      \
        ++vtxIter;                                                             \
    }
...
void makeSphere(float radius, int slices, int stacks, VtxOutIter vtxIter,
    ...
    for (int i = 0; i < slices + 1; ++i) {
        for (int j = 0; j < stacks + 1; ++j) {
            ...
            *vtxIter = GenericVertex(x * radius, y * radius, z * radius, x, y,
                                     z, 1.0 / slices * i, 1.0 / stacks * j,
